...
Kept out of ``sim.py`` on the same discipline the reference used: rendering
must never happen inside the simulation lock for longer than it takes to copy
the arrays out.  All the real image work lives here, on the caller's thread,
against those copies.

The palette climbs monotonically in luminance from end to end so density stays
readable even after a viewer flattens colour to grey.  A positive-charge overlay
is warm and a negative-charge overlay is cool, chosen against the dark
background so the sign is obvious at a glance.
"""
...
# Perceptually monotone ramp; the same one that the reference calls "true", so
# a screenshot flattened to grey preserves the density ordering.
_RAMP_STOPS = [(0.00, (16, 24, 52)), (0.22, (30, 82, 140)),
...
def render_png(occ: np.ndarray, paint: np.ndarray | None = None,
               max_px: int = 900, smooth: float = 1.0) -> bytes:
    ...
    lut = (BG[None, :] * (1.0 - lo[:, None]) + RAMP * lo[:, None]) \
        .astype(np.uint8)
    img = lut[idx]

    if paint is not None and float(np.abs(paint).max()) > 1e-9:
